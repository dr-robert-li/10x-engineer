...
test('install + uninstall surgical-removal round-trip — user-owned sibling untouched', async (t) => {
  ...
  // 10 response-mode files materialised at <project>/.clinerules/10x-engineer/
  // (build-mode-overview is routed through BUILD_MODE_INSTRUCTION + persona.txt
  // — never installed as a standalone .md by per-skill adapters.)
