...
//   3. git log --all (commit messages + author identity).
//
// Exit codes:
//   0 — clean
...
// ---- 6. Audit git log ------------------------------------------------------
function auditGitLog(forbid, hits) {
  ...
  // 6b. Commit body audit — every forbidden pattern scanned across full log.
  ...
  for (const pattern of forbid) {
    ...
    while ((m = re.exec(fullLog)) !== null) {
      // git log has no allowlist — any hit is contamination.
