...
const PROMPT_FILENAME = '10x-engineer.md';
...
// Derive the prompts/<name>.md path from a homedir. Returns the path the
// adapter will write/read regardless of whether the home dir exists yet.
function promptPathFor(homedir) {
  return join(homedir, '.codex', 'prompts', PROMPT_FILENAME);
}
...
export default {
  ...
  async install({ skills, scope, paths, dryRun = false, version }) {
    ...
    // Slash-command file — global only. paths.global is <homedir>/.codex/AGENTS.md;
    // siblings/prompts/10x-engineer.md is its peer. Skip on project-only scope.
    if ((scope === 'global' || scope === 'both') && paths.global) {
      const command = await loadCommand();
      const promptPath = join(paths.global, '..', 'prompts', PROMPT_FILENAME);
      if (!dryRun) await mkdir(join(paths.global, '..', 'prompts'), { recursive: true });
      await safeWriteFile(promptPath, codexPromptBody(command), { dryRun });
  ...
  async uninstall({ scope, paths, dryRun = false }) {
    ...
    // Slash-command file — global only.
    if ((scope === 'global' || scope === 'both') && paths.global) {
      const promptPath = join(paths.global, '..', 'prompts', PROMPT_FILENAME);
