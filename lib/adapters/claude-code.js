...
// copies the runtime hook scripts (lib/hooks/session-start.js,
// lib/hooks/user-prompt-submit.js) into <homedir>/.claude/hooks/, writes
...
const HOOK_SCRIPT_FILENAMES = {
  sessionStart: '10x-engineer-session-start.js',
  userPromptSubmit: '10x-engineer-user-prompt-submit.js',
};
...
function packageHookSourcePath(scriptFilename) {
  ...
  // into hooks/. We pass plain filenames (session-start.js / user-prompt-submit.js).
...
// Build the persona.txt content — concatenated skill bodies, identical
// shape to the output-style body but without frontmatter.
function buildPersonaText(skills) {
...
export default {
  ...
  async install({ skills, scope, paths, dryRun = false, version }) {
    ...
    if ((scope === 'global' || scope === 'both') && paths.global) {
      ...
      // (10x-engineer-session-start.js / 10x-engineer-user-prompt-submit.js).
      const srcSessionStart = packageHookSourcePath('session-start.js');
      const srcUserPromptSubmit = packageHookSourcePath('user-prompt-submit.js');
      if (!dryRun) {
      ...
      // session-start hook reads at runtime.
      await safeWriteFile(installed.persona, buildPersonaText(skills), { dryRun });
