...
import { writeState, statePath, clearState } from './state.js';
...
/**
 * Ensure a fresh install lands in the default-off state. HOOK-01 / HOOK-08:
 *   - state.json absent → write {enabled:false}
 *   - state.json present (any value) → preserve verbatim (no clobber, no read)
 *   - dryRun:true       → no-op
 *
 * Called from runInstall AFTER consent + harness selection and BEFORE the
 * adapter loop. The check is `access(F_OK)` rather than `readState` because
 * we never need to know the current value — we either preserve it or
 * create-as-disabled. This keeps the orchestrator agnostic of state shape.
 *
 * @param {{ homedir: string, dryRun: boolean }} ctx
 * @returns {Promise<{ wrote: boolean, path: string, reason?: string }>}
 */
async function ensureFreshInstallDefaultsToDisabled({ homedir, dryRun }) {
  const file = statePath({ homedir });
  if (dryRun) return { wrote: false, path: file, reason: 'dry-run' };

  try {
    await access(file, fsConstants.F_OK);
    return { wrote: false, path: file, reason: 'pre-existing-state' };
  } catch (e) {
    if (e.code !== 'ENOENT') throw e;
  }

  const r = await writeState({ homedir, enabled: false, dryRun: false });
  return { wrote: !!r.written, path: file };
}
...
export async function runInstall({
  ...
  // HOOK-01: default-off no-clobber. Fresh machines get state.json with
  // enabled:false; re-installs preserve any existing state verbatim.
  await ensureFreshInstallDefaultsToDisabled({ homedir, dryRun });
