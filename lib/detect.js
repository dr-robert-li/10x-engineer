...
// The registry is loaded dynamically inside detectAll() rather than as a
// top-level import. Reason: six adapters import findAncestorWith /
// commandExists from this module, and the registry imports those adapters,
// so a static `import adapters from './adapters/index.js'` here creates an
// ESM cycle that hits the TDZ when any adapter is loaded standalone (e.g.
// from an adapter unit test). Dynamic import inside the function defers
// binding resolution until module evaluation has finished. node's module
// cache makes the per-call await effectively free after the first call.
// The registry array reference is preserved, so test code that mutates the
// array (e.g. throwing-adapter isolation in test/install.test.js) continues
// to work.
...
export async function detectAll(ctx) {
  const { default: adapters } = await import('./adapters/index.js');
